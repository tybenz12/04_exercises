...
```{r}
tys_places <- tibble(
  place = c("Sandy Bum Exuma", "Joss Cafe & Sushi Bar","Severn School", "Home", "Oriole Park at Camden Yards", "College Apartment", "Leonard Center", "Mac Baseball Field", "US Bank Stadium", "Target Field"),
  long = c(-75.840458, -76.490295, -76.547325, -76.538761, -76.622494, -93.164040, -93.167584, -93.169762, -93.259766, -93.277864),
  lat = c(23.549924, 38.977882, 39.069023, 39.095945, 39.283812, 44.934030, 44.937651, 44.935212, 44.974228, 44.981685), 
  top_three = c(TRUE, FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, FALSE, FALSE, FALSE))

pal <- colorFactor(c("lightblue", "red"),
                    domain = c(TRUE, FALSE))
...
num_depart <- departures[match(unique(departures$sstation), departures$sstation),]

pal <- colorNumeric("magma",
                    domain = num_depart$count)
...
num_casual <- casual_rider[match(unique(casual_rider$sstation), casual_rider$sstation),]

pal <- colorNumeric("magma",
                    domain = num_casual$percent_casual)

leaflet(num_casual) %>% 
  addProviderTiles(providers$Esri.WorldImagery) %>% 
  addCircles(lng = ~long,
             lat = ~lat,
             color = ~pal(percent_casual),
             label = ~sstation, 
             opacity = .8, 
             fillOpacity = .8) %>% 
  addLegend("topleft",
            ...
            opacity = 1, 
            title = "Percentage of Casual Riders")
```
...
max_cases <- covid[match(unique(covid$state), covid$state),]

final_covid_data <- max_cases %>% 
  mutate(name = str_to_lower(state))

pal <- colorNumeric("magma",
                    domain = final_covid_data$cases)
